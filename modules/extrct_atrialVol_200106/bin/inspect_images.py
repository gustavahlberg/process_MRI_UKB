...
for t in range(T):
    area, length, landmarks = evaluate_atrial_area_length(
        seg_la_2ch[:, :, 0, t], nim_2ch, long_axis)
    if type(area) == int:
        if area < 0:
            continue
...
if not atrium_pass_quality_control(seg_la_4ch, {'LA': 1, 'RA': 2}):
    print('{0} seg_la_4ch does not atrium_pass_quality_control.'
          .format(data))
...
for t in range(T):
    labels = seg_la_4ch[:, :, 0, t]
    area, length, landmarks = evaluate_atrial_area_length(labels,
                                                          nim_4ch,
                                                          long_axis)
    if type(area) == int:
        if area < 0:
    ...
    V['LA_bip'][t] = 8 / (3 * math.pi) * area[0] * \
                     A['LA_2ch'][t] / (0.5 * (length[0] + L['LA_2ch'][t]))
    A['RA_4ch'][t] = area[1]
    L['RA_4ch'][t] = length[1]
    ...
    if t == 0:
        ...
        writer = vtk.vtkPolyDataWriter()
        writer.SetInputData(poly)
        writer.SetFileName('{0}/lm_la_4ch_{1:02d}.vtk'
                           .format(data_dir, t))
        writer.Write()
