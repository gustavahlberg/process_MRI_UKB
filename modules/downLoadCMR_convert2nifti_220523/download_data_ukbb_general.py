...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "hd:u:k:c:i", 
                                   ["data_root=", "util_dir=",
                                    "ukbkey=", "csv_file=", "instance="])
    except getopt.GetoptError:
        print('download_data_ukbb_general.py') 
        print('-d <data_root> -u <util_dir> -k <ukbkey> -c <csv_file> -i <instance>')
        sys.exit(2)
    ...
    for i in range(start_idx, end_idx):
        ...
        with open(batch_file, 'w') as f_batch:
            for j in range(20208, 20211):
                ...
                # 20210: Aortic distensibilty images - DICOM
                ...
                field = '{0}-{1}.0'.format(j, ins)
                f_batch.write('{0} {1}_{2}_0\n'.format(eid, j, ins))

        # Download the data using the batch file
        ukbfetch = os.path.join(util_dir, 'ukbfetch')
        logfile = os.path.join("log", eid)
        print('{0}: Downloading data for subject {1} ...'.format(i, eid))
        os.system('{0} -b{1} -a{2} -o{3}'.format(ukbfetch, batch_file, ukbkey, logfile))
        ...
        os.system('rm {0}_20208*.zip'.format(eid))
        os.system('rm {0}_20209*.zip'.format(eid))
        os.system('mv {0}_*.zip {1}'.format(eid, data_dir))


        dl = open("downloadList.txt", "a")
        dl.write('downloaded sample id: {0}, into: {1} \n'.format(eid, data_dir))
        dl.close()
